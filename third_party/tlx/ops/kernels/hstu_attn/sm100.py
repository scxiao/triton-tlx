...
Not self-contained: `_util.py` and `_stubs.py` are helpers, and `_reference.py`
is the plain-Triton HSTU promoted whole because this kernel imports five
`@triton.jit` helpers from it.

All three autotuners are applied lazily so the search space can vary per caller.
"""
...
def tlx_hstu_attention_bwd(
    ...
    if use_persistent:
        ...
        bwd_kernel = _tuned_bwd(_SPACE, True)
    else:
        grid = lambda meta: (  # noqa E731
            H * Z * triton.cdiv(max_seq_len, meta["BLOCK_N1"]), )
        bwd_kernel = _tuned_bwd(_SPACE, False)
...
@functools.lru_cache(maxsize=None)
def _tuned_bwd(space, persistent):
    """Autotuned backward kernel for a search space and persistence mode."""
    kernel = _hstu_attn_bwd_ws if persistent else _hstu_attn_bwd_ws_non_persistent
    return triton.autotune(
        configs={"full": BWD_CONFIGS_FULL, "smoke": BWD_CONFIGS_SMOKE}[space](),
        key=["AUTOTUNE_MAX_SEQ_LEN", "HEAD_DIM"],
    )(kernel)

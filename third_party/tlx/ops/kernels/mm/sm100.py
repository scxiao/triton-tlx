"""Blackwell (sm100) GEMM -- the `tlx.ops.mm` implementation.

Promoted from `tutorials/blackwell_gemm_ws.py`, which is now frozen.

Autotune is applied lazily (`_tuned`) rather than as a module-scope decorator,
so the search space can vary per caller. The tutorial's explicit-config launch
...
#: Full autotune search space. Used by the perf caller (``space="full"``).
...
def _smoke_configs():
    ...
    variations = [
        {},
        {"EPILOGUE_SUBTILE": 2},
        {"NUM_MMA_GROUPS": 2, "BLOCK_SIZE_M": 256},
        {"SPLIT_K": 2},
        {"NUM_CTAS": 2},
        {"INTERLEAVE_EPILOGUE": 1, "USE_WARP_BARRIER": True},
    ]
    out = []
...
def heuristic_config(M, N, K):
    ...
    cfg = get_heuristic_config(M, N, K, _get_num_sms())
    if cfg is None:
        return None
    cfg = dict(cfg)
...
def _widen_on_empty(prune, full_space):
    ...
    `preprocess_configs` is shape-dependent, so no fixed subset is guaranteed to
    survive it; without this the failure is an opaque "No valid autotuner
    configs after pruning".
    """
...
def _tuned(space, shape=None):
    ...
    return triton.autotune(
        configs=configs,
        key=["M", "N", "K"],
        prune_configs_by={"early_config_prune": _widen_on_empty(preprocess_configs, CONFIGS)},
...
def mm(a, b, *, space="full"):
    ...
    config) for correctness, "smoke" for path coverage. Not exposed on
    `tlx.ops.mm`.
    """
